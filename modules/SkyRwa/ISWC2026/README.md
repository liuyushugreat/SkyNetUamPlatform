...
| 7 | `reproduce_shacl_engines.py` | Table 10 rudof columns + Fig. 2 (§7.5) | rudof ~16× faster end-to-end at 1 000 flights; both engines conform |
| 8 | `reproduce_robustness.py` | §7.6 | identical counts across 5 seeds; threshold sweep 7.6–22.9% |
| 9 | `reproduce_competency.py` | Table 11 (§7.7) | **12/12 CQs correct** on the enriched 9 035-triple audit graph |
...
| 12 | `reproduce_ontology_quality.py` | §4.5 + Appendix A (Table 12) | pitfall scan, OWL DL consistency, 12-CQ construct mapping |
...
|----|----------------|:------:|:-----:|:---------:|:--------:|
| V1 | Missing evidence digest | – | YES | YES | YES |
...
| | **Detection rate** | **50%** | **50%** | **100%** | **100%** |

\* via a `sh:sparql` conditional constraint.

### Table 11 — Competency-question verification (§7.7)
