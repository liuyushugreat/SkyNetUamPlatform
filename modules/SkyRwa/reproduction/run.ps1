...
$steps = @(
    ...
    @("reproduce_table7.py",           "Validation-layer coverage analysis (Tables 8-9)"),
    @("reproduce_scoring_context.py",  "Scoring-context materialization cost (Table 10, ~6 min)"),
    @("reproduce_table8.py",           "Runtime overhead 5-1000 flights (Table 11)"),
    @("reproduce_shacl_engines.py",    "Dual-engine SHACL comparison (Table 11 + Fig. 2)"),
    @("reproduce_robustness.py",       "Robustness: seeds, scale, thresholds (Sect. 7.6)"),
    @("reproduce_competency.py",       "Competency questions CQ1-CQ12 (Table 12)"),
